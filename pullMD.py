...
class YoudaoNotePull(object):
    ...
    def _covert_config(self, config_path=None) -> (dict, str):
        ...
        key_list = ['local_dir', 'local_dir_meta', 'ydnote_dir', 'smms_secret_token', 'is_relative_path']
        if key_list != list(config_dict.keys()):
            return {}, '请检查「config.json」的 key 是否分别为 local_dir, local_dir_meta, ydnote_dir, smms_secret_token, is_relative_path'
        return config_dict, ''
